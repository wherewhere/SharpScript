    ...
    public sealed partial class RoslynCodeSession : ICodeSession
    {
        ...
        public RoslynCodeSession(SourceText code, RoslynOptions options, bool isConsole, ILogger<RoslynCodeSession>? logger = null)
        {
            ...
            Solution solution = Workspace.CurrentSolution
                ...
                .AddDocument(docId, "Program.cs", code);
        ...
        private static async Task<MetadataReferenceHost?> GetMetadataReferenceAsync(string assembly, IDictionary<string, string> fingerprinting, HttpClient client, ILogger<RoslynCodeSession> logger, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                {
                    Task<XmlDocumentationProvider?> documentTask = CreateDocumentationAsync(client, assembly, logger, cancellationToken);
                    ...
                    static async Task<XmlDocumentationProvider?> CreateDocumentationAsync(HttpClient client, string assembly, ILogger<RoslynCodeSession> logger, CancellationToken cancellationToken = default)
                    {
                        ...
                        try
                        {
                            byte[] bytes = await client.GetByteArrayAsync(xml, cancellationToken).ConfigureAwait(false);
                            if (bytes?.Length > 0)
                            {
                        ...
                        catch (Exception ex)
                        {
                            ...
                            if (_baseUrl != baseUrl)
                            {
                                try
                                {
                                    ...
                                    byte[] bytes = await _client.GetByteArrayAsync(xml, cancellationToken).ConfigureAwait(false);
                                    if (bytes?.Length > 0)
                                    {
                                        return XmlDocumentationProvider.CreateFromBytes(bytes);
                                    }
            ...
            if (references.Count > 0)
            {
                ...
                MetadataReferenceHost?[] results = await Task.WhenAll(references.Select(async path =>
                {
                    if (!string.IsNullOrEmpty(path) && !References.Any(x => path.Equals(x.Display, StringComparison.OrdinalIgnoreCase)))
                    {
                        ...
                        return index != -1 ? _addon[index] : await GetMetadataReferenceAsync(path, _fingerprinting!, client, _logger, cancellationToken);
                    }
                    return null;
                })).ConfigureAwait(false);
